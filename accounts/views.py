            ...
            if user is not None:
                ...
                if user.is_staff == False :
                    ...
                    if P.has_started == False:
                        ...
                        ul.save()
                    # print(4)
                return redirect('quiz/instructions')
            else:
...
def register(request):
    if request.method == "POST":
        ...
        if not re.search(regexpassword,password):
            messages.error(request,"The password must contain at least 8 characters,1 uppercase character,1 speacial character and a number.")
            return render(request,'register.html',{'title':'register'})
        ...
        if password == cpassword:
            if User.objects.filter(username=username).exists():
                ...
                user.save()
                messages.success(request,'Signedup successfully, Now Login pls')
...
def check(request):
    ...
    if user.exists():
        m = "Username ' "+ name +" ' already exist. You cannot use it."
    else:
        m = "Username ' "+ name +" ' do not exist. You can use it."
